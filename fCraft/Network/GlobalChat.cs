...
namespace fCraft
{
    public static class GlobalChat
    {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        Logger.Log(LogType.SystemActivity,
                                    "Connecting to MyCraft Global Chat as {2}",
                                    hostName, port, ActualBotNick);
                        if (ActualBotNick == "[[MyCraft]DefaultServer]")
                        {
            ...
            void HandleMessage([NotNull] string message)
            {
                ...
                switch (msg.Type)
                {
                    ...
                    case IRCMessageType.Join:
                        ...
                        {
                            SendList.Message("&i[Global] Server {0} joined the MyCraft Global Chat",
                                            msg.Nick);
                        }
                        else
                        {
                            SendList.Message("&i[Global] {0} joined the MyCraft Global Chat",
                                            msg.Nick);
                        }
                        return;
                    ...
                    case IRCMessageType.Quit:
                        if (!ResponsibleForInputParsing) return;
                        SendList.Message("&i[Global] Server {0} left the MyCraft Global Chat",
                                        msg.Nick);
                        return;
                    ...
                    case IRCMessageType.Error:
                        ...
                        switch (msg.ReplyCode)
                        {
                            ...
                            case IRCReplyCode.ErrorBadChannelKey:
                                Logger.Log(LogType.SystemActivity,
                                            "Error: Channel password required for {0}. MyCraft does not currently support passworded channels.",
                                            msg.Channel);
                                die = true;
                                GCReady = false;
                                break;
        ...
        public static void Init()
        {
            ...
            port = 6667;
            channelNames = new[] { "#MyCraft.global" };
            for (int i = 0; i < channelNames.Length; i++)
            {
