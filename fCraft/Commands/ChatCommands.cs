...
namespace fCraft
{
    static class ChatCommands
    {
        ...
        static readonly CommandDescriptor CdGlobal = new CommandDescriptor
                {
                    ...
                    IsConsoleSafe = true,
                    Permissions = new[] { Permission.Chat },
                    Usage = "&a/Global [help/rules/accept/disconnect/<message to send>]",
                    Help = "Sends a global message to other MyCraft servers",
                    Handler = GHandler
                };
        ...
        static void GHandler(Player player, Command cmd)
        {
            ...
            if ((!player.GlobalChat) && (Msg == "rules"))
            {
            ...
            else if ((player.GlobalChat) && (Msg == "rules"))
            {
                player.Message(" &cRules: No spamming and no advertising. All chat rules that apply to your server apply here.\n" +
                    "&eServer staff have the right to kick you.\n" +
                    "&9By using the Global Chat, you accept these conditions.");
            ...
            else if ((!player.GlobalChat) && (Msg == "accept"))
            {
            ...
            else if ((player.GlobalChat) && (Msg == "disconnect"))
            {
            ...
            if ((!player.GlobalChat) && ((Msg.Length < 1) || (Msg.Length > 1)))
            {
                player.Message("&WYou must read and accept the global chat rules. Type &a/Global rules.");
                return;
            }

            else if ((player.GlobalChat) &&((Msg == "") || (Msg == null)))
            {
