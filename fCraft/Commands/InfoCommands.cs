    ...
    static class InfoCommands {
        ...
        public static void PrintPlayerInfo ( [NotNull] Player player, [NotNull] PlayerInfo info ) {
           ...
           /* string HasPaidSite = "https://minecraft.net/haspaid.jsp?user=" + info.Name;
            ...
            string HasPaidResponseFromServer = HasPaidResults.ReadToEnd();
            if (HasPaidSite.Contains("@"))
            {
                HasPaidResponseFromServer = "Email";
            }
            else if (HasPaidResponseFromServer == "true")
            {
                HasPaidResponseFromServer = "Premium";
            }
            else if (HasPaidResponseFromServer == "false")
            {
                HasPaidResponseFromServer = "Free";
            }
            else
            {
        ...
        internal static void ServerInfoHandler ( Player player, Command cmd ) {
            ...
            if ( MonoCompat.IsMono ) {
                player.Message( "  Running &6My&8Craft&S {0}, under Mono {1}",
                                Updater.CurrentRelease.VersionString,
                                MonoCompat.MonoVersionString );
            } else {
                player.Message( "  Running &6My&8Craft&S {0}, under .NET {1}",
                                Updater.CurrentRelease.VersionString,
                                Environment.Version );
            }
            ...
            player.Message( "  Tracking {0} total players ({1} online, {2} banned ({3:0.0}%), {4} IP-banned).",
        ...
        internal static void HelpHandler ( Player player, Command cmd ) {
            ...
            if ( commandName == "commands" ) {
                ...
                player.Message( "  To list available ranks, write &H/Ranks");
        ...
        static void LastCMDHandler(Player player, Command cmd)
        {
            ...
            else
            {
                ...
                {
                    {
                        if (Server.LastCMD.Count == 0)
                        {
                            player.Message("&SNo commands have been executed on the server yet");
                            return;
                        }
                        player.Message("&SThese are the last commands executed on the server");
                        foreach (String lastcmd in Server.LastCMD)
                        {
                            player.Message("&H{0}", lastcmd);
                        }
                    }
                }
                else
                {
                    {
                        ...
                        if (target.PlayerObject.LastCMD.Count == 0)
                        {
                            player.Message("&SNo commands have been executed yet");
                            return;
                        }
        ...
        {
            Name = "Devs",
            Aliases = new[] { "developers, developer, dev" },
            Category = CommandCategory.Info,
            IsConsoleSafe = false,
            Permissions = new[] { Permission.Chat },
            Usage = "&a/Devs",
            Help = "&SShows a list of software developers.",
            Handler = DevsHandler
        };
