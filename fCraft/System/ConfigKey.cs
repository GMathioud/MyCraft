...
namespace fCraft
{
    ...
    public enum ConfigKey
    {
        ...
        [StringKey(ConfigSection.General, "[MyCraft] Default Server",
...
@"Port number on your local machine that MyCraft uses to listen for
...
overwhelming the Internet connection with data.",
        ...
        [BoolKey(ConfigSection.General, true,
@"Enables Global Chat (/Global) for IRC channel #MyCraft.global on esper.net")]
        GCKey,
...
@"Require players to specify a reason/memo when promoting or demoting someone.")]
        ...
        [BoolKey(ConfigSection.Security, false,
@"Announce a message to players with Ban permission that a banned player tried to login.")]
        BannedPlayerMsg,
...
@"Maximum number of backup files that MyCraft should keep.
...
@"MyCraft includes an IRC (Internet Relay Chat) client for
...
@"IRC bot's nickname. If the nickname is taken, MyCraft will append
...
@"MyCraft Crash Reporter is under development. For now, you'll have to send the error to Predator121.
Email address of the developer: Predator121@MyCraft-Servers.tk
Thank you.")]
        SubmitCrashReports,

        [EnumKey(ConfigSection.Advanced, fCraft.UpdaterMode.Prompt,
@"MyCraft can automatically update to latest stable versions.
...
@"It is recommended to leave MyCraft at default priority.
Setting this below ""Normal"" may starve MyCraft of resources.
...
@"The rate at which MyCraft applies block updates, in milliseconds. Lowering this will slightly
...
MyCraft bind to the same IP every time.")]
        IP,
